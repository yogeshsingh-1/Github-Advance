<!-- In mongo shell ObjectId() -->
ObjectId()  ->In mongoShell ObjectId Type ka Function exist karta hai.
ishe ObjectId() function ki value mai 2 character 1 byte bnate hai.

starting ke 4 bytes time ko represnet karte hai

test> ObjectId()
ObjectId('6a4cf920dec6832808f04324')
...
In mongo Long Datatype Store like string but threat as an number :-
...
db.users.find({info : { $type : "array" }})
db.users.find({info : { $type : "object" }})
db.users.find({info : { $type : "date" }})
db.users.find({info : { $type : "string" }})
