...
var mid = "Resell00448805757124";
var key = "KXHUJH&Ywq9pUkkr";
  ...
  try {
    ...
    if (isVerifySignature) {
      ...
      if (req.body.STATUS === "TXN_SUCCESS") {
        return res.redirect(
          `http://localhost:3000/success?orderId=${ORDERID}&message=${RESPMSG}`,
        );
      } else {
        return res.redirect(
          `http://localhost:3000/failure?orderId=${ORDERID}&message=${RESPMSG}`,
        );
      }
    } else {
...
router.post("/payment", async (req, res) => {
   ...
   */
  var paytmChecksum = await PaytmChecksum.generateSignature(params, key);
  let paytmParams = {
    ...params,
